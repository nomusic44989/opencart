...
class ModelCatalogProduct extends Model {
	public function addProduct(array $data): int {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` SET `product_id` = '" . (int)$product_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($value['name']) . "', `shotname` = '" . $this->db->escape($value['shotname']) . "', `description` = '" . $this->db->escape($value['description']) . "', `tag` = '" . $this->db->escape($value['tag']) . "', `meta_title` = '" . $this->db->escape($value['meta_title']) . "', `meta_description` = '" . $this->db->escape($value['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
			...
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					...
					foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
						$this->db->query("INSERT INTO `" . DB_PREFIX . "product_attribute` SET `product_id` = '" . (int)$product_id . "', `attribute_id` = '" . (int)$product_attribute['attribute_id'] . "', `language_id` = '" . (int)$language_id . "', `text` = '" . $this->db->escape($product_attribute_description['text']) . "', `main` = '" . (int)$product_attribute['main'] . "', `sort_order` = '" . (int)$product_attribute['sort_order'] . "'");
					}
				}
			}
		}
		...
		if (!empty($data['desc'])) {
			foreach ($data['desc'] as $key => $product_desc) {
				if (!empty($product_desc)) {
				  foreach ($product_desc as $language_id => $product_descs) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_desc` SET `product_id` = '" . (int)$product_id . "', `description_id` = '" . (int)$key . "', `language_id` = '" . (int)$language_id . "', `name` = '" .  $this->db->escape($product_descs['name']) . "'");
				  }
				}
			}
		}
	...
	public function copyProduct(int $product_id): void {
		...
		if ($product_info) {
			...
			$product_data['desc'] = $this->model_catalog_product->getDesc($product_id);
	...
	public function addVariant(int $master_id, array $data): int {
		...
		if ($master_info) {
			...
			// Desc
			if (!isset($override['desc'])) {
				$product_data['desc'] = $this->model_catalog_product->getDesc($master_id);
			}
		}
	...
	public function getProducts(array $data = []): array {
		...
		if (isset($data['filter_show']) && $data['filter_show'] == 'sku') {
			$sql .= " AND p.`sku` IS NOT NULL";
		}
		
		if (isset($data['filter_show']) && $data['filter_show'] == 'mpn') {
			$sql .= " AND p.`mpn` IS NOT NULL";
		}
	...
	public function getAttributes(int $product_id): array {
        ...
        $product_attribute_query = $this->db->query("SELECT `attribute_id` FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = '" . (int)$product_id . "' GROUP BY `attribute_id` ORDER BY `sort_order` ASC");
        ...
        foreach ($product_attribute_query->rows as $product_attribute) {
            ...
            foreach ($product_attribute_description_query->rows as $product_attribute_description) {
				...
				$main = $product_attribute_description['main'];
				$sort_order = $product_attribute_description['sort_order'];
            }
	...
	public function getProductDesc(int $product_id): array {
		$product_desc_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_desc` WHERE `product_id` = '" . (int)$product_id . "' AND `language_id` = '" . (int)$this->config->get('config_language_id') . "'");
		
		return $product_desc_query->rows;
	}
	
	public function getDesc(int $product_id): array{
		$product_desc_data = [];
		$product_desc_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_desc` WHERE `product_id` = '" . (int)$product_id . "'");
		
		foreach ($product_desc_query->rows as $product_description) {
			$product_desc_data[$product_description['description_id']][$product_description['language_id']] = ['name' => $product_description['name']];
		}
		return $product_desc_data;
	}
	...
	public function getReports($product_id, $start = 0, $limit = 10) {
		if ($start < 0) {
			$start = 0;
		}
	...
	public function sqlSaveBarcode(int $product_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "product` SET `sku` = '" . $this->db->escape($data['sku']) . "', `mpn` = '" . $this->db->escape($data['mpn']) . "', `price` = '" . $this->db->escape($data['price']) . "', `date_modified` = NOW() WHERE product_id = '" . (int)$product_id . "'");
		
		if(!empty($data['product_description'])){
			foreach ($data['product_description'] as $language_id => $value) {
				$this->db->query("UPDATE `" . DB_PREFIX . "product_description` SET `shotname` = '" . $this->db->escape($value['shotname']) . "' WHERE `product_id` = '" . (int)$product_id . "' and `language_id` = '" . (int)$language_id . "'");
			}
		}
	}

	public function getTotalProductsByOptionValueId(int $option_value_id) {
